...
<template>
  <div class="space-y-6 space-y-6">
    ...
    <!-- 成绩趋势图 -->
    <div class="bg-white dark:bg-dark-surface rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50">
      <div class="flex flex-wrap gap-3 justify-between items-center mb-4">
        <h3 class="font-semibold text-gray-800 dark:text-gray-200">成绩趋势</h3>
        <div class="flex gap-2">
          <select v-model="selectedCategory" class="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-surface text-sm">
            <option v-for="cat in categories" :key="cat" :value="cat">{{ cat }}</option>
          </select>
          <select v-model="selectedSubject" class="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-surface text-sm">
    ...
    <div class="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200/50 dark:border-gray-700/50 flex justify-between items-center">
        ...
        <button @click="showAddModal = true" class="px-4 py-2 rounded-xl bg-gradient-to-r from-brand-500 to-purple-500 text-white text-sm font-medium hover:shadow-lg transition">
          + 添加成绩
        </button>
      </div>
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="showAddModal = false">
      <div class="bg-white dark:bg-dark-surface rounded-2xl w-full max-w-md">
        <div class="p-6">
          ...
          <div class="space-y-4">
            <div>
              ...
              <select v-model="newScore.subjectName" class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-surface">
                <option v-for="subject in subjects" :key="subject" :value="subject">{{ subject }}</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">分数</label>
              <input v-model.number="newScore.score" type="number" min="0" max="100" class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-surface">
            </div>
            <div>
              <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">考试日期</label>
              <input v-model="newScore.examDate" type="date" class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-surface">
            </div>
          </div>
          <div class="flex gap-3 mt-6">
            <button @click="showAddModal = false" class="flex-1 px-4 py-2 rounded-xl border border-gray-200 text-gray-600">取消</button>
            <button @click="addScore" class="flex-1 px-4 py-2 rounded-xl bg-brand-500 text-white">确认添加</button>
          </div>
        </div>
      </div>
    </div>
...
const initChart = () => {
  if (trendChartRef.value) {
    ...
    trendChart.setOption({
      ...
      series: [{ type: 'line', data: [], smooth: true, lineStyle: { color: getCssColor('--theme-primary', '#d946ef'), width: 3 }, areaStyle: { opacity: 0.2 } }]
    })
    fetchTrend()
  }
}
