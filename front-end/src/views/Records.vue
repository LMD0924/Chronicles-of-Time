<!--
  文件说明：拾光记前台应用拾光记项目页面组件，承载拾光记项目场景的界面展示、交互操作和数据承接。
-->
<script setup>
...
const fetchRecords = async () => {
  loading.value = true
  try {
    const res = await request.post('/growth/list', queryParams.value)
    if (res.code === 200) {
...
const fetchStats = async () => {
  const res = await request.get('/growth/stats')
