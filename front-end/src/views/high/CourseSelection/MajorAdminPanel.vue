    ...
    <el-dialog v-model="detailVisible" title="专业详情" width="800px" :class="{ 'dark-dialog': isDark }">
      ...
      <template #footer>
        <div class="dialog-footer">
          <button class="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700" @click="detailVisible = false">关闭</button>
        </div>
      </template>
    </el-dialog>
  </div>
...
:deep(.el-tag--double-first-class) {
...
const getUniversityLevelClass = (level) => {
  if (!level) return ''
  if (level.includes('985')) return 'text-amber-600 font-semibold'
  if (level.includes('211')) return 'text-emerald-600 font-semibold'
  if (level.includes('双一流')) return 'text-blue-600 font-semibold'
  return ''
}
</script>
