  ...
  <div class="cs-panel space-y-4">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <select v-model="form.firstSubjectName" class="w-full px-3 py-2 rounded-lg border dark:bg-dark-surface dark:border-slate-600">
      ...
      <div>
        ...
        <select v-model="form.secondSubject1Name" class="w-full px-3 py-2 rounded-lg border dark:bg-dark-surface dark:border-slate-600">
          <option value="">请选择</option>
      ...
      <div>
        ...
        <select v-model="form.secondSubject2Name" class="w-full px-3 py-2 rounded-lg border dark:bg-dark-surface dark:border-slate-600">
          <option value="">请选择</option>
          <option v-for="subject in secondSubjects" :key="subject.id" :value="subject.name">
            {{ subject.name }}
          </option>
      ...
      <div>
        ...
        <input v-model="form.selectionReason" class="w-full px-3 py-2 rounded-lg border dark:bg-dark-surface dark:border-slate-600" />
      </div>
    </div>
    <div class="flex justify-end gap-2">
      <button @click="emit('cancel')" class="px-4 py-2 rounded-lg border">取消</button>
      <button @click="submit" :disabled="saving" class="px-4 py-2 rounded-lg bg-brand-600 text-white">
        {{ saving ? '保存中...' : '保存' }}
      </button>
    </div>
  </div>
