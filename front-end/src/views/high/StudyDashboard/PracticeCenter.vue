<template>
  <div class="space-y-6">
    <!-- 练习头部 - 考试信息栏 -->
    <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl p-6 border border-white/20 dark:border-gray-700/30 shadow-2xl shadow-indigo-500/10">
      <div class="flex flex-wrap justify-between items-center gap-4">
        ...
        <div class="flex gap-3 flex-wrap">
          <select v-model="examType" class="px-5 py-2.5 rounded-2xl border-0 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm text-sm font-medium shadow-sm hover:shadow-md transition-all focus:ring-2 focus:ring-indigo-400/50">
    ...
    <!-- 主内容区：左右布局 -->
    <div v-if="questions.length > 0" class="grid grid-cols-1 lg:grid-cols-4 gap-6">
      <!-- 左侧：题号导航区 -->
            ...
            <div class="grid grid-cols-5 gap-2">
              <button
                ...
                :class="[
                  ...
                  !isAnswered(question.id) && !isMarked(question.id) ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' : '',
          ...
          <div v-if="showAnswerDetails" class="p-4 border-t border-gray-200/50 dark:border-gray-700/50">
            ...
            <div class="mt-4 space-y-1">
              <div v-for="(question, idx) in questions" :key="question.id" class="flex items-center gap-2 py-1">
                ...
                <span class="text-xs truncate flex-1">{{ question.questionTitle?.substring(0, 30) || '' }}{{ question.questionTitle?.length > 30 ? '...' : '' }}</span>
      ...
      <div class="lg:col-span-3">
        <div class="bg-white dark:bg-gray-800/50 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
          <!-- 题目头部 -->
          <div class="p-5 border-b border-gray-200/50 dark:border-gray-700/50 bg-gradient-to-r from-indigo-50/30 to-purple-50/30 dark:from-transparent dark:to-transparent">
          ...
          <div class="p-6">
            ...
            <!-- 选项区域 -->
            <div v-if="currentQuestion.questionType !== '解答' && currentQuestion.questionType !== '填空'" class="space-y-3 mb-6">
              <div
              ...
              >
                <div :class="[
                  ...
                  <span v-if="currentQuestion.questionType === '多选' ? userAnswers[currentQuestion.id]?.includes(getOptionValue(option)) : userAnswers[currentQuestion.id] === getOptionValue(option)" class="text-white text-xs">✓</span>
                </div>
            ...
            <div v-else class="mb-6">
              <textarea
                ...
                rows="5"
                class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              ></textarea>
            </div>

            <!-- 解析区域（提交后显示） -->
            <div v-if="submittedQuestions[currentQuestion.id]" class="mt-6 p-4 rounded-xl" :class="isAnswerCorrect(currentQuestion) ? 'bg-green-50 dark:bg-green-950/20' : 'bg-red-50 dark:bg-red-950/20'">
              <div class="flex items-start gap-3">
                <span class="text-2xl">{{ isAnswerCorrect(currentQuestion) ? '✅' : '❌' }}</span>
                <div class="flex-1">
                  <p class="font-medium" :class="isAnswerCorrect(currentQuestion) ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'">
          ...
          <!-- 底部导航按钮 -->
          <div class="p-5 border-t border-gray-200/50 dark:border-gray-700/50 bg-gray-50/50 dark:bg-gray-900/30 flex justify-between">
            <button
              @click="prevQuestion"
              :disabled="currentIndex === 0"
              class="px-5 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-800 transition"
            >
              ← 上一题
            </button>
            <button
              @click="submitCurrentAnswer"
              class="px-5 py-2 rounded-xl bg-indigo-500 text-white hover:bg-indigo-600 transition"
            >
              提交本题
            </button>
            <button
    ...
    <div v-if="showScratchPaper" class="fixed inset-0 z-50" @click.self="toggleScratchPaper">
      ...
      >
        <!-- 标题栏（可拖动） -->
        <div 
          class="px-4 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 flex justify-between items-center cursor-move select-none"
...
const scratchPaperSize = reactive({ width: 800, height: 600 })
...
const submitAllMistakes = async () => {
  ...
  for (const question of questions.value) {
    ...
    if (isCorrect) continue // 正确的题目不加入错题本

    if (addedToMistake.value[question.id]) continue // 已加入的跳过
...
const initScratchCanvas = () => {
  ...
  canvas.height = scratchPaperSize.height - 48 // 减去标题栏高度
